...
const cmd: CMD = {
  ...
  exec: (message: Message, d: any, args: Array<string>) => {
    if (
      ...
      const shadow_emb = (new d.MessageEmbed() as MessageEmbed)
        .setTitle("Sent to the shadow realm!")
        .setImage(
          "https://33.media.tumblr.com/a1be537496b06043da405bc6c2117da0/tumblr_nslr77kZbz1qzh2aio8_500.gif"
        )
        .setColor("#000000");
      ...
      if (target_member?.roles.cache.has("845381923941253120")) {
        return message.channel.send(
          "Thou canst banish this guardian to the shadow realm."
        );
      }

      if (member_role) target_member?.roles.remove(member_role);

      if (shadowRealm_vc) target_member?.voice.setChannel(shadowRealm_vc);
