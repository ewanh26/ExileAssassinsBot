...
const cmd = {
    ...
    exec: (message, d, args) => {
        if (message.member?.roles.cache.some(r => r.id === "845381923941253120" || r.id === "845008330208706620")) {
            const release_emb = new d.MessageEmbed()
                .setTitle("Released from the shadow realm!")
                .setImage("http://pa1.narvii.com/6573/49565d33d2984d78fbb7724c74bac0f258d21d8b_00.gif")
                .setColor("#000000");
            ...
            if (shadowRealm_vc)
                target_member?.voice.setChannel(null);
            if (member_role)
                target_member?.roles.add(member_role);
            release_emb.setDescription(`${target_member || "someone"} has been released from the shadow realm.`);
            message.channel.send(release_emb);
        }
        else {
            return message.channel.send("Thou canst release them from the shadow realm");
        }
    },
